...
open class BaseDeviceInfo
{
    companion object {
        ...
        fun getDeviceInfoList(context: Context): ArrayList<InfoModel>{
            val androidSdk = context.getString(R.string.android_sdk, Build.VERSION.RELEASE, BaseDeviceInfo().androidVersion, Build.VERSION.SDK_INT)
            val manufacturer = BaseDeviceInfo().uiFramework
            val cpu = FrameworkZips.ARCH

            val initBootState = BaseDeviceInfo().determineVerifiedBootState(context)
            val verifiedBootDeactivated = initBootState.first
            val verifiedBootExplanation = initBootState.second

            val list = ArrayList<InfoModel>()
            list.add(InfoModel(deviceSdk, R.drawable.ic_android, androidSdk, ""))
            list.add(InfoModel(deviceManufacturer, R.drawable.ic_phone, manufacturer, ""))
            list.add(InfoModel(deviceCpu, R.drawable.ic_chip, cpu, ""))
            if (verifiedBootDeactivated.isNotEmpty() || verifiedBootExplanation.isNotEmpty()) {
                list.add(InfoModel(deviceVerified, R.drawable.ic_verified, verifiedBootDeactivated, verifiedBootExplanation))
            }
            return list
        }
    }
