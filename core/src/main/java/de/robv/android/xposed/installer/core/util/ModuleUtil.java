...
public final class ModuleUtil {
    ...
    private final BaseXposedApp mApp;
    ...
    public static final String SETTINGS_CATEGORY = BASE_PKG +".category.MODULE_SETTINGS";

    private ModuleUtil() {
        mApp = BaseXposedApp.getInstance();
    ...
    public synchronized void updateModulesList(boolean showToast) {
        try {
            Log.i(BaseXposedApp.TAG, "updating modules.list");
            int installedXposedVersion = BaseXposedApp.getInstalledXposedVersion();

            PrintWriter modulesList = new PrintWriter(MODULES_LIST_FILE);
            PrintWriter enabledModulesList = new PrintWriter(new BaseXposedApp().ENABLED_MODULES_LIST_FILE);
            ...
            FileUtils.setPermissions(new BaseXposedApp().ENABLED_MODULES_LIST_FILE, 00664, -1, -1);

            if (showToast)
                showToast(R.string.xposed_module_list_updated);
        } catch (IOException e) {
            Log.e(BaseXposedApp.TAG, "cannot write " + MODULES_LIST_FILE, e);
    ...
    public class InstalledModule {
        ...
        public Drawable getIcon() {
            ...
            mIntent.addCategory(SETTINGS_CATEGORY);
