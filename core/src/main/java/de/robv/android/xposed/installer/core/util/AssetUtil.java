...
public class AssetUtil {
    public static final File BUSYBOX_FILE = new File(BaseXposedApp.getInstance().getCacheDir(), "busybox-xposed");
    ...
    public static File writeAssetToFile(AssetManager assets, String assetName, File targetFile, int mode) {
        try {
            if (assets == null)
                assets = BaseXposedApp.getInstance().getAssets();
            InputStream in = assets.open(assetName);
            writeStreamToFile(in, targetFile, mode);;
            return targetFile;
        } catch (IOException e) {
            Log.e(BaseXposedApp.TAG, "could not extract asset", e);
            if (targetFile != null)
                targetFile.delete();
    ...
    @SuppressWarnings({"ConstantConditions", "OctalInteger"})
